...
export const RoadmapDatabaseNode = memo(({ data, selected }: NodeProps) => {
    ...
    return (
        <div className="relative group flex flex-col items-center">
            ...
            {/* Handles */}
            <Handle type="target" position={Position.Top} className="!bg-slate-500 !w-2 !h-2 !border-0 opacity-0 group-hover:opacity-100 transition-opacity !top-2" />
            <Handle type="source" position={Position.Bottom} className="!bg-slate-500 !w-2 !h-2 !border-0 opacity-0 group-hover:opacity-100 transition-opacity" />
            <Handle type="target" position={Position.Left} className="!bg-slate-500 !w-2 !h-2 !border-0 opacity-0 group-hover:opacity-100 transition-opacity !top-14" />
            <Handle type="source" position={Position.Right} className="!bg-slate-500 !w-2 !h-2 !border-0 opacity-0 group-hover:opacity-100 transition-opacity !top-14" />
        </div>
    );
});
