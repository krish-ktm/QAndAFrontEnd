...
interface QuizSectionDesktopProps {
    productId: string;
}
...
export const QuizSectionDesktop = ({ productId }: QuizSectionDesktopProps) => {
    // Flow state
                        ...
                        <div className="space-y-4">
                            ...
                            {quizzes.map((quiz, index) => {
                                ...
                                return (
                                    <div
                                        key={quiz.id}
                                        className={`bg-white rounded-xl shadow-sm border-2 p-6 ${isCorrect ? 'border-green-200' : 'border-red-200'
                                            }`}
                                    >
                                        {/* Question Header */}
                                        <div className="flex items-start justify-between mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                                    }`}>
                                                    {index + 1}
                                                </div>
                                                <h3 className="text-lg font-semibold text-gray-900">Question {index + 1}</h3>
                                            </div>
                                            <div className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${isCorrect ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                                }`}>
                                                {isCorrect ? (
                                                    <>
                            ...
                            {currentQuiz.options.map((option, index) => {
                                ...
                                return (
                                    <button
                                        ...
                                        className={`flex items-center p-4 border rounded-lg transition ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:bg-gray-50'
                                            }`}
                                    >
